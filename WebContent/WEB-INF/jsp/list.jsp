		...
		<table style="width:100%;">
			...
			<s:iterator value="items">
				<tr>
					...
					<s:if test="status==0">
						<td>已上架</td>
						<td><a href='/items/down.action?item.id=${id}&item.status=<s:property value="#parameters['item.status][0]'" />'>下架</a></td>
					</s:if>
					<s:else>
						<td>已下架</td>
						<td><a href='/items/up.action?item.id=${id}&item.status=<s:property value="#parameters['item.status'][0]" />'>上架</a></td>
					</s:else>
				</tr>
			</s:iterator>
		</table>
